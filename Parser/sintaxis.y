...
statement: 
    ID ASIGNACION REF ID ';' {
                                ...
                                printf("[Parser] Base Constraint: %s ⊇ {%s}\n", $1, $4);
                                $$ = s;
                             }
    | ID ASIGNACION ID ';'   {
                                OpSeq *s = opSeq_new();
                                ...
                                printf("[Parser] Simple Constraint: %s ⊇ %s\n", $1, $3);
                             }
    | ID ASIGNACION DEREF ID ';' {
                                OpSeq *s = opSeq_new(); 
                                opSeq_push(s, op_c1($1,$4)); 
                                $$=s;
                                printf("[Parser] Complex 1 Constraint: %s ⊇ *%s\n", $1, $4);
                             }
    | DEREF ID ASIGNACION ID ';' {
                                OpSeq *s = opSeq_new(); 
                                opSeq_push(s, op_c2($2,$4)); 
                                $$=s;
                                printf("[Parser] Complex 2 Constraint: *%s ⊇ %s\n", $2, $4);
                             }
