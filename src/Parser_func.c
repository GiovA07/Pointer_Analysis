...
static void eval_stmt(Graph **G, Op *s, StateTable *st)
{
    ...
    {
    case OP_BASE:
        constraitBase(G, s->a, s->b);
        break; // a = &b
    case OP_SIMPLE:
        constraintSimple(G, s->a, s->b);
        break; // a = b
    case OP_C1:
        constraintComplex1(G, s->a, s->b);
        break; // a = *b
    case OP_C2:
        constraintComplex2(G, s->a, s->b);
        break; // *a = b
    case OP_IF:
    {
        printf("[Operator] IF\n");
...
/*
 * Procesa una unica instruccion y registra su estado para su visualizacion.
 * Toma el grafo antes (IN) y despues (OUT) de la evaluacion,
 * guardandolos en la StateTable junto con el texto de la operacion.
 */
